...
public class NetManagerWalker : MonoBehaviour
{
    ...
    [Range(1f, 8)]
    public ProtectedFloat timeScale = 1;

    public ProtectedFloat topDistance;

    public Material[] placingMaterials;

    public ProtectedFloat timer;
    ...
    public void SaveToJsona()
    {
        ...
        try
        {
            using (FileStream fs = new FileStream(Application.dataPath + "/PlayerDataddFile.json", FileMode.Create)) //change to something more usable in future
                bf.Serialize(fs, nets[nets.Count - 1].weights);
        }
        catch
        {
            using (FileStream fs = new FileStream(Application.dataPath + "/PlayerDataddFile.json", FileMode.CreateNew)) //change to something more usable in future
                bf.Serialize(fs, nets[nets.Count - 1].weights);
        }
