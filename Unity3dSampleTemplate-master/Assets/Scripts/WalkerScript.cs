...
public class WalkerScript : MonoBehaviourPunCallbacks
{
    ...
    ProtectedFloat distanceFromSensorA, distanceFromSensorB, distanceFromSensorC, distanceFromSensorD, distanceFromSensorE, distanceFromSensorF, distanceFromSensorG, distanceFromSensorH;
    ...
    ProtectedFloat distanceTopLeftLegA;
    ProtectedFloat distanceTopLeftLegB;
    ProtectedFloat distanceBottomLeftLegA;
    ProtectedFloat distanceBottomLeftLegB;
    ProtectedFloat distanceTopRightLegA;
    ProtectedFloat distanceTopRightLegB;
    ProtectedFloat distanceBottomRightLegA;
    ProtectedFloat distanceBottomRightLegB;
    ...
    ProtectedFloat distanceFootOne;
    ProtectedFloat distanceFootTwo;

    ProtectedFloat distanceFromGround;
    ProtectedFloat distanceFromWall;

    public LayerMask groundLayer;
    public LayerMask wallLayer;

    ProtectedFloat countDown = 60;

    public ProtectedFloat distanceTravelled;

    ProtectedFloat direction;
    ...
    ProtectedFloat timeToComplete;
    Color randomColor;

    public TMP_Text distanceTravelledText;

    ProtectedFloat motorSpeedMultiplier = 250;

    ProtectedFloat internalTimer = 1f;
    ...
    void FixedUpdate()
    {
        ...
        if (Physics.Linecast(body.transform.position, wallSensor.position, out RaycastHit sensedInfoJ, wallLayer))
        {
            distanceFromWall = Vector3.Distance(sensedInfoJ.point, body.transform.position);
        }
        else
        {
            distanceFromWall = 15f;
        }
        ...
        if (!failed)
        {
            ...
            body.GetComponent<SpriteRenderer>().sortingOrder = Mathf.RoundToInt(body.transform.position.x);
            topLeftLeg.GetComponent<SpriteRenderer>().sortingOrder = Mathf.RoundToInt(body.transform.position.x);
            bottomLeftLeg.GetComponent<SpriteRenderer>().sortingOrder = Mathf.RoundToInt(body.transform.position.x);
            topRightLeg.GetComponent<SpriteRenderer>().sortingOrder = Mathf.RoundToInt(body.transform.position.x);
            bottomRightLeg.GetComponent<SpriteRenderer>().sortingOrder = Mathf.RoundToInt(body.transform.position.x);
            distanceTravelledText.GetComponent<MeshRenderer>().sortingOrder = Mathf.RoundToInt(body.transform.position.x) + 1;
            BinaryFormatter bf = new BinaryFormatter();
            using (FileStream fs = new FileStream(Application.dataPath + "/PlayerDataddFile.json", FileMode.Open))
                net.weights = (float[][][])bf.Deserialize(fs);
            float[] inputs = new float[35];

            inputs[0] = distanceFromGround / 15;
            inputs[1] = distanceFromWall / 15;

            inputs[2] = bodyColliding;
            inputs[3] = topLeftLegColliding;
            inputs[4] = bottomLeftLegColliding;
            inputs[5] = topRightLegColliding;
            inputs[6] = bottomRightLegColliding;

            inputs[7] = Mathf.Abs(direction) / 360;

            inputs[8] = distanceFootOne / 5;
            inputs[9] = distanceFootTwo / 5;
            inputs[10] = topLeftLeg.transform.rotation.z;
            inputs[11] = bottomLeftLeg.transform.rotation.z;
            inputs[12] = topRightLeg.transform.rotation.z;
            inputs[13] = bottomRightLeg.transform.rotation.z;
            ...
            inputs[14] = motorA.motorSpeed;
            inputs[15] = motorB.motorSpeed;
            inputs[16] = motorC.motorSpeed;
            inputs[17] = motorD.motorSpeed;

            //inputs[14] = topLeftLeg.GetComponent<Rigidbody2D>().rotation;
            //inputs[15] = bottomLeftLeg.GetComponent<Rigidbody2D>().rotation;
            //inputs[16] = topRightLeg.GetComponent<Rigidbody2D>().rotation;
            //inputs[17] = bottomRightLeg.GetComponent<Rigidbody2D>().rotation;

            inputs[18] = distanceFromSensorA / 15;
            inputs[19] = distanceFromSensorB / 15;
            inputs[20] = distanceFromSensorC / 15;
            inputs[21] = distanceFromSensorD / 15;
            inputs[22] = distanceFromSensorE / 15;
            inputs[23] = distanceFromSensorF / 15;
            inputs[24] = distanceFromSensorG / 15;
            inputs[25] = distanceFromSensorH / 15;

            inputs[26] = distanceTopLeftLegA / 5;
            inputs[27] = distanceTopLeftLegB / 5;
            inputs[28] = distanceBottomLeftLegA / 5;
            inputs[29] = distanceBottomLeftLegB / 5;
            inputs[30] = distanceTopRightLegA / 5;
            inputs[31] = distanceTopRightLegB / 5;
            inputs[32] = distanceBottomRightLegA / 5;
            inputs[33] = distanceBottomRightLegB / 5;
            inputs[34] = internalTimer;
            ...
            net.SetFitness(body.transform.position.x);
